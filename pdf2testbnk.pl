...
# June 2014, Ice
# don't forget to put (\d\.\d-) at the EOF
...
open(FILE, '>files\out.txt');
	...
	if ($data[$line_num] =~m/^Multiple-Choice Questions/) 
		...
		elsif ($data[$line_num] =~m/^\d\d\.\d\-/)					# find questions
			{$data[$line_num] =~s/\n/ /;
			$data[$line_num] =~s/\d\d\.\d\-//;					# remove chapter number in question number
		...
		$e_nxt_line = $line_num + 1;							# parse multi-line choice
		  ...
		  print FILE "\n$data[$line_num]";						
			...
			while ($data[$ratl_nxt_line] !~ /^\d\d\.\d\-/)		# parse multi-line rationale
